...
class RoundRobin:
    ...
    def select_server(self):
        serverSelected = self.countServer%len(self.servers)
        self.countServer += 1
        return self.servers[serverSelected]
...
class LeastConnections:
    ...
    def select_server(self):
        serverSelected = min(self.connections, key=self.connections.get)
        self.connections[serverSelected] = self.connections[serverSelected] + 1
        
        for i in self.servers:
            if i[1] == serverSelected:
                return i
...
class LeastResponseTime:
    ...
    def select_server(self):
        while True:
            ...
            serverSelected = min(self.avg_time.values())
            ...
            if(self.server_time[self.servers[self.currentServer][1]] == serverSelected):
                break
        
        self.server_time[serverSelected] = time.time()
