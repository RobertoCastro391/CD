...
class FingerTable:
    ...
    def __init__(self, node_id, node_addr, m_bits=10):
        ...
        for k in range(self.m_bits):
            self.idx_list.append((k+1, (self.node_id + pow(2,k)) % pow(2, self.m_bits)))

    def fill(self, node_id, node_addr):
        """ Fill all entries of finger_table with node_id, node_addr."""
        for k in range(self.m_bits):
            self.fingerTable[k] = (node_id, node_addr)
    ...
    def find(self, identification):
        ...
        for k in range(len(self.fingerTable)):
            if contains(self.node_id, self.fingerTable[k][0], identification):
                return self.fingerTable[k-1][1]
        return self.fingerTable[len(self.fingerTable)-1][1]
    ...
    def refresh(self):
        ...
        for k in range(len(self.fingerTable)):
            index = (self.node_id + pow(2,k)) % pow(2, self.m_bits)
            entryFingerTable.append((k+1, index, self.fingerTable[k][1]))
        
        return entryFingerTable
...
class DHTNode(threading.Thread):
    ...
    def node_join(self, args):
        ...
        if self.identification == self.successor_id:  # I'm the only node in the DHT
            ...
            self.finger_table.fill(self.identification, addr)
    ...
    def get_successor(self, args):
        ...
        if (contains(self.identification, self.successor_id, args["id"])):
            self.send(args["from"], {"method": "SUCCESSOR_REP", "args": {"id": args["id"],"successor_id": self.successor_id, "successor_addr": self.successor_addr}})
        elif (self.predecessor_id is None or contains(self.predecessor_id, self.identification, args["id"])):
            self.send(args["from"], {"method": "SUCCESSOR_REP", "args": {"id": args["id"], "successor_id": self.identification, "successor_addr": self.addr}})
        else:
    ...
    def put(self, key, value, address):
        ...
        if contains(self.identification, self.successor_id, key_hash):
            ...
            addressToSend = self.finger_table.find(key_hash)
            self.send(addressToSend, {"method": "PUT", "args": {"key": key, "value": value, "from": address}})
    ...
    def get(self, key, address):
        ...
        if contains(self.identification, self.successor_id, key_hash): 
            ...
            addressToSend = self.finger_table.find(key_hash)
            self.send(addressToSend, {"method": "GET", "args": {"key": key, "from": address}})
        ...
        while not self.done:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REQ":
                    ...
                    self.finger_table.update(self.finger_table.getIdxFromId(output["args"]["id"]), output["args"]["successor_id"], output["args"]["successor_addr"])
